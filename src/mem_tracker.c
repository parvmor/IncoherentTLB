...
static ssize_t memtrack_read(struct file *filp, char *buffer, size_t length,
                             loff_t *offset) {
    return handle_read(buffer, length);
}

static ssize_t memtrack_write(struct file *filp, const char *buff, size_t len,
                              loff_t *off) {
...
int init_module(void) {
    ...
    memtrack_device =
        device_create(memtrack_class, NULL, MKDEV(major, 0), NULL, DEVNAME);
